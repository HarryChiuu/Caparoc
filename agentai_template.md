...
# Step 2: 初始化 git 儲存庫
git init
git config --local user.name "AgentAI" (請修改Author名稱)
git config --local user.email "agent@ai.com" (請修改Author Github E-mail)
...
git init && git config --local user.name "AgentAI" && git config --local user.email "agent@ai.com"
