...
**實機驗證結果（192.168.50.111，2026-09-04）**：回傳 `applied=True`，顯示
「✓ 已寫入並生效」，重新整理頁面後設備上仍是新名稱。
→ **這台韌體的 Attr 6 立即生效，不需要重啟設備。**

`applied=False`（需重啟）那條路徑因此在本機未被觸發過，屬防禦性處理——
保留是因為 EDS 未載明此行為，不能假設所有韌體版本都一樣。
