...
"""
CAPAROC 簡化四通道控制器
從 caparoc_unified_edit.py 中提取的 full_four_channel_control 功能
專注於 Implicit Messaging 模式的可靠控制

功能:
- Implicit Messaging 連接建立
- 四通道獨立控制 (開啟/關閉)
- 即時狀態監控
- 互動式命令介面

使用方法:
    python caparoc_simple_control.py
"""
...
class CaparocSimpleController:
    ...
    def _implicit_io_worker(self, driver, verbose=True):
        ...
        while self.cip_keep_alive and self.implicit_mode_enabled:
            try:
                ...
                try:
                    # 嘗試寫入 Output Assembly (使用測試程式的方法)
                    ...
                    if cycle % 500 == 0:  # 減少錯誤輸出頻率
                        print(f"[Implicit I/O] 週期 {cycle} 錯誤: {io_e}")
    ...
    def read_voltage(self, driver=None):
        """讀取系統電壓 (使用 Assembly.101)"""
        if driver is None:
            driver = self.driver
        try:
            response = driver.read("Assembly.101[4]")
    ...
    def read_total_current(self, driver=None):
        """讀取總電流 (使用 Assembly.101)"""
        if driver is None:
            driver = self.driver
        try:
            # 讀取總電流
            i_response = driver.read("Assembly.101[6]")
    ...
    def read_channel_current(self, driver=None, module=1, channel=1, debug=False):
        """讀取指定通道電流 (直接讀取 Assembly.101)"""
        if driver is None:
            driver = self.driver
        try:
            offset = 20 + (module - 1) * 16 + (channel - 1) * 2
            response = driver.read(f"Assembly.101[{offset}]")
            if response and hasattr(response, 'value'):
